...
class ParaverbalAnalyzer:
    ...
    async def analyze(
        ...
        *,
        ...
        y, sr = librosa.load(io.BytesIO(audio_bytes), sr=self._target_sr, mono=True)
        duration = float(librosa.get_duration(y=y, sr=sr))

        # Silence detection — librosa default top_db=60 is too aggressive for speech.
        intervals = librosa.effects.split(y, top_db=30)
        voiced_seconds = float(sum(end - start for start, end in intervals)) / sr
        ...
        f0_voiced: np.ndarray = f0[~np.isnan(f0)] if f0 is not None else np.array([])
